import { LICENSE_CODE_RAW_LENGTH, LICENSE_PAYLOAD_LENGTH, SAFE_ALPHABET, } from './constants.js';
const SAFE_ALPHABET_SET = new Set(SAFE_ALPHABET);
export class LicenseValidator {
    ...
    static calculateChecksumChar(base15) {
        if (base15.length !== LICENSE_PAYLOAD_LENGTH)
            return '';
        let weightedSum = 0;
        for (let index = 0; index < base15.length; index += 1) {
            ...
            if (!character)
                return '';
            const alphabetIndex = SAFE_ALPHABET.indexOf(character);
            if (alphabetIndex === -1)
                return '';
